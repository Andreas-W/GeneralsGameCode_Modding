...
typedef std::map< NameKeyType, const ObjectCreationList* > UnitOCLMap;
...
private:
	...
	void parseOCLEntry(INI* ini, void* instance, void* store, const void* /*userData*/);
	// TODO

	VecObjectPtr& getPortableStructures(void);
	VecObjectID  m_portableStructureIDs;
	...
	// How do we track slot positions?
	// Should portable structures be a fixed size Array, so their slot position is clear?
	// Or instead have a separate Vector that tracks the slot positions?

	bool createOCLForRider(Object* obj);
};
